...
*   Note that the XR app's GPU budget is LESS than the display period. Always subtract the surfaceflinger GpuMSPF when determining the app's budget. The GPU cost of compositing varies depending on the app frame resolution and content.
...
*   Bottom row (nominal case): This row shows the longest sequence of no frame drops for comparison with the problem rows.
...
*   We may not see the SysUI GPU usage, so if there is any evidence of SysUI rendering in --frame-stats or --top then that could explain frame drops where it appears the app CPU and GPU usage is okay. If so, and this is not intended, then trace again without SysUI activity.
...
*.  RunnableMSPF shows the MS per frame that the process threads were in runnable state. This includes background threads that are not aligned with per-frame code, so it may not always correlate well with frame drop cases where the time per frame is higher than the nominal case.
...
*   These numbers can occasionally be corrupted, so if there are clearly wrong values then suggest restarting the app and try another trace.
